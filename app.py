...
GOOGLE_FONTS = "https://fonts.googleapis.com/css2?family=Chakra+Petch:wght@400;500;600;700&family=Inter:wght@400;500;600;700&display=swap"
# Fuente de todo el sitio (Inter como fallback)
FONT_DISPLAY = "'Chakra Petch', 'Inter', sans-serif"
...
KPI_VAL_STYLE = {"fontSize": "1.25rem", "fontWeight": "bold", "color": TEXT_MAIN, "margin": "0", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}
KPI_LBL_STYLE = {"fontSize": "0.68rem", "color": COLOR_NEUTRAL, "textTransform": "uppercase", "letterSpacing": "1px", "marginTop": "6px", "fontWeight": "bold", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}
...
TAB_STYLE = {
    ...
    "borderBottom": f"1px solid {BORDER_COLOR}", "padding": "15px 10px", "fontSize": "0.85rem", "fontWeight": "600", "letterSpacing": "0.5px", "cursor": "pointer", "fontFamily": FONT_DISPLAY
}
TAB_SELECTED_STYLE = {
    "backgroundColor": BG_COLOR, "color": TEXT_MAIN, "border": "none",
    "borderBottom": f"3px solid {COLOR_POS}", "padding": "15px 10px", "fontSize": "0.85rem", "fontWeight": "600", "letterSpacing": "0.5px", "cursor": "pointer", "fontFamily": FONT_DISPLAY
}

# Botones de período de Performance: estilo base y activo
PERF_BTN_STYLE = {"fontFamily": "'Chakra Petch', 'Inter', sans-serif", "fontWeight": "bold", "borderColor": BORDER_COLOR, "color": COLOR_NEUTRAL}
...
CUSTOM_GRID_STYLE = {
    ...
    "--ag-font-family": "'Chakra Petch', 'Inter', sans-serif",
    "--ag-font-size": "13px",
    "--ag-checkbox-checked-color": COLOR_NEUTRAL,
    "--ag-range-selection-border-color": "transparent"
}
INPUT_STYLE = {"backgroundColor": BG_COLOR, "color": TEXT_MAIN, "border": f"1px solid {BORDER_COLOR}", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "boxShadow": "none"}
DROPDOWN_STYLE = {"backgroundColor": BG_COLOR, "color": "#000", "border": f"1px solid {BORDER_COLOR}", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}
...
def get_analytics_figures(df_closed, df_open, start_bal, user_config, selected_metric, cash_movements_df=None):
    empty = {"layout": {"xaxis": {"visible": False}, "yaxis": {"visible": False}, "plot_bgcolor": "rgba(0,0,0,0)", "paper_bgcolor": "rgba(0,0,0,0)",
                        "annotations": [{"text": "Sin datos — registrá tus primeros trades en OPERATIVA", "showarrow": False,
                                         ...
                                         "font": {"color": COLOR_NEUTRAL, "family": "Chakra Petch, Inter, sans-serif", "size": 13}}]}}
...
# --- EVOLUCION DEL EDGE (ESPERANZA MATEMATICA) ---
        ...
        if selected_metric and selected_metric in df_closed:
            ...
            a_label_font = dict(color=TEXT_MAIN, size=11, family="'Chakra Petch', 'Inter', sans-serif")
...
def get_management_panel():
    return html.Div(id="management-container", style={'display': 'none'}, children=[
        dbc.Card([
            dbc.CardHeader("PANEL DE GESTION DE RIESGO", style={"backgroundColor": "transparent", "borderBottom": f"1px solid {BORDER_COLOR}", "fontWeight": "bold", "color": TEXT_MAIN, "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}), 
            dbc.CardBody([
                html.Div(id="dyn-info", className="mb-3 fw-bold", style={"color": TEXT_MAIN, "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}), 
                dbc.Tabs([
                    dbc.Tab(label="CERRAR POSICION", children=[
                        ...
                        dbc.Textarea(id="c-notes", placeholder="Notas de salida / Lecciones aprendidas...", style={"backgroundColor": BG_COLOR, "color": TEXT_MAIN, "border": f"1px solid {BORDER_COLOR}", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "marginBottom": "15px", "height": "80px"}),
                        dbc.Button("LIQUIDAR TOTALIDAD", id="btn-close", color="danger", className="w-100 fw-bold", style={"backgroundColor": COLOR_NEG, "border": "none", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"})
                    ...
                    dbc.Tab(label="TOMA PARCIAL", children=[
                        ...
                        dbc.Button("EJECUTAR PARCIAL", id="btn-part", color="light", className="w-100 fw-bold text-dark", style={"border": "none", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"})
                    ], style=TAB_STYLE, tab_style=TAB_STYLE, active_tab_style=TAB_SELECTED_STYLE), 
                    
                    dbc.Tab(label="AJUSTAR SL", children=[
                        dbc.InputGroup([
                            dbc.InputGroupText("NUEVO SL", style={"backgroundColor": BORDER_COLOR, "color": COLOR_NEUTRAL, "border": "none", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}),
                            dbc.Input(id="usl", type="number", style=INPUT_STYLE),
                            dbc.Button("ACTUALIZAR", id="btn-sl", color="light", className="text-dark", style={"fontFamily": "'Chakra Petch', 'Inter', sans-serif"})
                        ], className="my-3")
                    ], style=TAB_STYLE, tab_style=TAB_STYLE, active_tab_style=TAB_SELECTED_STYLE),
                    dbc.Tab(label="BORRAR TODO", children=[
                        html.Div([
                            dcc.ConfirmDialogProvider(
                                children=dbc.Button("ELIMINAR TODAS LAS POSICIONES (RESET)", id="btn-del-all-open", color="danger", className="w-100 fw-bold", style={"backgroundColor": "rgba(246, 70, 93, 0.2)", "color": COLOR_NEG, "border": f"1px solid {COLOR_NEG}", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}),
                    ...
                    dbc.Tab(label="BORRAR OPERACIÓN", children=[
                        html.Div([
                            dbc.Button("ELIMINAR REGISTRO INDIVIDUAL (DB)", id="btn-del", color="dark", size="sm", className="w-100 mt-2 text-danger border-danger", style={"fontFamily": "'Chakra Petch', 'Inter', sans-serif"})
                        ], className="my-3")
                    ], style=TAB_STYLE, tab_style=TAB_STYLE, active_tab_style=TAB_SELECTED_STYLE)
                ])
            ])
...
global_modals = html.Div([
    dbc.Modal([dbc.ModalHeader("REGISTRO DE USUARIO", style={"backgroundColor": CARD_BG, "color": TEXT_MAIN, "borderBottom": f"1px solid {BORDER_COLOR}", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}), dbc.ModalBody([
        ...
        ], style={"display": "flex", "alignItems": "center", "flexWrap": "wrap", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}),
        html.Div(id="reg-msg", className="text-danger mt-2")
    ], style={"backgroundColor": CARD_BG}), dbc.ModalFooter([dbc.Button("CANCELAR", id="close-reg", color="dark", className="ms-auto", style={"fontFamily": "'Chakra Petch', 'Inter', sans-serif"}), dbc.Button("ACEPTAR", id="do-reg", color="success", style={"backgroundColor": COLOR_POS, "border": "none", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "color": "#000"})], style={"backgroundColor": CARD_BG, "borderTop": f"1px solid {BORDER_COLOR}"})], id="modal-reg", is_open=False),
    dbc.Modal([
        dbc.ModalHeader("TÉRMINOS Y CONDICIONES", style={"backgroundColor": CARD_BG, "color": TEXT_MAIN, "borderBottom": f"1px solid {BORDER_COLOR}", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}),
        dbc.ModalBody([
        ...
        ], style={"backgroundColor": CARD_BG, "color": COLOR_NEUTRAL, "maxHeight": "60vh", "overflowY": "auto", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}),
        dbc.ModalFooter(dbc.Button("CERRAR", id="close-tc", color="dark", style={"fontFamily": "'Chakra Petch', 'Inter', sans-serif"}), style={"backgroundColor": CARD_BG, "borderTop": f"1px solid {BORDER_COLOR}"})
    ], id="modal-tc", is_open=False, size="lg", scrollable=True),
    dbc.Modal([
        dbc.ModalHeader("RECUPERAR CONTRASEÑA", style={"backgroundColor": CARD_BG, "color": TEXT_MAIN, "borderBottom": f"1px solid {BORDER_COLOR}", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}),
        dbc.ModalBody([
            html.P("Ingresá tu usuario y el email con el que te registraste. Si coinciden, vas a poder definir una contraseña nueva.", style={"color": COLOR_NEUTRAL, "fontSize": "0.85rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}),
            ...
            html.Div(id="rec-msg", className="mt-2", style={"fontSize": "0.85rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "minHeight": "20px"})
        ], style={"backgroundColor": CARD_BG}),
        dbc.ModalFooter([
            dbc.Button("CANCELAR", id="close-recovery", color="dark", className="ms-auto", style={"fontFamily": "'Chakra Petch', 'Inter', sans-serif"}),
            dbc.Button("RESTABLECER", id="do-recovery", color="success", style={"backgroundColor": COLOR_POS, "border": "none", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "color": "#000"})
        ], style={"backgroundColor": CARD_BG, "borderTop": f"1px solid {BORDER_COLOR}"})
    ], id="modal-recovery", is_open=False),
    dbc.Modal([dbc.ModalHeader("CONFIGURACION DE ESTRATEGIA", style={"backgroundColor": CARD_BG, "color": TEXT_MAIN, "borderBottom": f"1px solid {BORDER_COLOR}", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}), dbc.ModalBody([html.P("Definí parámetros para clasificar cada operación según el motivo por el cual fue tomada (ej: Trend Following, Buy & Hold, Swing Trading). Esto permite distinguir los trades entre las distintas variantes de operatoria y analizar el desempeño de cada una por separado en la pestaña de Analytics. (Ej: Parámetro: Trend Following — Opciones: MA Crossover, ATH, BreakOut)", style={"color": COLOR_NEUTRAL, "fontSize": "0.82rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "marginBottom": "15px"}), dag.AgGrid(id="conf-grid", columnDefs=[{"field": "Parametro", "editable": True}, {"field": "Opciones", "editable": True, "flex": 1}], rowData=[], dashGridOptions={"rowSelection": "single", "stopEditingWhenCellsLoseFocus": True}, className="ag-theme-alpine-dark", style={"height": "300px", "borderRadius": "4px", **CUSTOM_GRID_STYLE}), dbc.Button("AGREGAR PARAMETRO", id="add-row-btn", color="dark", outline=True, size="sm", className="mt-3 w-100", style={"fontFamily": "'Chakra Petch', 'Inter', sans-serif"}), html.Hr(style={"borderColor": BORDER_COLOR}), dbc.Row([dbc.Col(dbc.Label("MONEDA DE LA CUENTA", className="fw-bold", style={"color": COLOR_NEUTRAL, "fontSize": "0.8rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "paddingTop": "8px"}), width=6), dbc.Col(dbc.Select(id="conf-currency", options=[{"label": "Dólar (US$)", "value": "USD"}, {"label": "Peso Argentino (AR$)", "value": "ARS"}], value="USD", style=INPUT_STYLE), width=6)]), html.P("Define el símbolo de los valores en toda la app. Para cuentas en pesos usá tickers de BYMA/CEDEARs con sufijo .BA (ej: GGAL.BA, AAPL.BA).", style={"color": COLOR_NEUTRAL, "fontSize": "0.75rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "marginTop": "8px", "marginBottom": "0"}), html.Div(id="config-feedback", className="mt-2 text-warning small")], style={"backgroundColor": CARD_BG}), dbc.ModalFooter([dbc.Button("CANCELAR", id="close-config", color="dark", className="ms-auto", style={"fontFamily": "'Chakra Petch', 'Inter', sans-serif"}), dbc.Button("GUARDAR", id="save-config", color="success", style={"backgroundColor": COLOR_POS, "border": "none", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "color": "#000"})], style={"backgroundColor": CARD_BG, "borderTop": f"1px solid {BORDER_COLOR}"})], id="modal-config", is_open=False, size="lg")
])
...
def layout_login():
    return html.Div(
        dbc.Card([
            dbc.CardBody([
                html.H2("EDGE JOURNAL", className="text-center mb-1", style={"color": TEXT_MAIN, "letterSpacing": "2px", "fontSize": "1.7rem", "fontWeight": "700", "fontFamily": FONT_DISPLAY, "textShadow": f"0 0 24px {COLOR_POS}44"}),
                html.P("Trading journal & analytics", className="text-center", style={"color": COLOR_NEUTRAL, "fontSize": "0.8rem", "letterSpacing": "1px", "marginBottom": "28px", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}),
                ...
                html.Div(id="login-msg", style={"color": COLOR_NEG, "fontSize": "0.85rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "minHeight": "24px", "marginBottom": "10px", "textAlign": "center"}),
                dbc.Button("INICIAR SESION", id="login-btn", color="success", className="w-100 mb-3 p-3 fw-bold", style={"backgroundColor": COLOR_POS, "color": "#000", "border": "none", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}),
                dbc.Button("Crear Cuenta", id="open-reg", color="link", className="w-100 text-decoration-none", style={"color": COLOR_NEUTRAL, "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}),
                dbc.Button("¿Olvidaste tu contraseña?", id="open-recovery", color="link", size="sm", className="w-100 text-decoration-none", style={"color": COLOR_NEUTRAL, "fontSize": "0.78rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"})
            ])
...
app.layout = html.Div([
    ...
    dbc.Container([
        dbc.Row([
            ...
            dbc.Col(html.H2("EDGE JOURNAL", className="my-4", style={"color": TEXT_MAIN, "fontWeight": "normal", "letterSpacing": "1px", "fontSize": "1.7rem", "fontWeight": "600", "fontFamily": FONT_DISPLAY, "textShadow": f"0 0 20px {COLOR_POS}33"}), width=8),
            dbc.Col([
                html.Div([
                    dbc.Button("CONFIG. ESTRATEGIA", id="open-config-btn", color="dark", className="me-3 fw-bold", style={"border": f"1px solid {BORDER_COLOR}", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}),
                    dbc.Button("SALIR", id="logout-btn", color="dark", outline=True, className="fw-bold", style={"fontFamily": "'Chakra Petch', 'Inter', sans-serif", "color": COLOR_NEUTRAL, "borderColor": BORDER_COLOR})
                ], id="header-actions", style={"display": "none"})
            ], width=4, className="mt-4 text-end")
        ]), 
        html.Div(id='page-content')
    ], fluid=True, style={"maxWidth": "1600px"})
], style={"backgroundColor": BG_COLOR, "minHeight": "100vh", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "color": TEXT_MAIN, "paddingBottom": "50px"})
...
def process_recovery(n_open, n_close, n_do, user, email, p1, p2):
    msg_err = {"color": COLOR_NEG, "fontSize": "0.85rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "minHeight": "20px"}
    msg_ok = {"color": COLOR_POS, "fontSize": "0.85rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "minHeight": "20px"}
...
def render_tab(tab, session):
    ...
    if tab == 'tab-active':
        ...
        return dbc.Row([
            dbc.Col([
                dbc.Card([
                    ...
                    dbc.CardBody([
                        ...
                        # --- NOTAS DE ENTRADA ---
                        dbc.Textarea(id="n-notes", placeholder="Notas de entrada...", style={"backgroundColor": BG_COLOR, "color": TEXT_MAIN, "border": f"1px solid {BORDER_COLOR}", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "marginBottom": "15px", "height": "80px"}),
                        
                        dbc.Button("EJECUTAR ORDEN", id="btn-new", color="light", className="w-100 mb-3 fw-bold text-dark", style={"border": "none", "padding": "12px", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}), 
                        dcc.Upload(id='upload-data', children=dbc.Button("IMPORTAR EXCEL", color="dark", outline=True, size="sm", className="w-100", style={"borderColor": BORDER_COLOR, "color": COLOR_NEUTRAL, "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}), multiple=False)
                    ])
            ...
            dbc.Col([
                dbc.Row([
                    dbc.Col(html.H5("POSICIONES ACTIVAS", className="mb-3", style={"color": TEXT_MAIN, "fontSize": "1.0rem", "fontWeight": "600", "letterSpacing": "1px", "fontFamily": FONT_DISPLAY}), width=8),
                    dbc.Col(dbc.RadioItems(
                        ...
                        style={"fontFamily": "'Chakra Petch', 'Inter', sans-serif"}
                ...
                dbc.Row([
                    dbc.Col(html.H5("EXPOSICION ACTUAL", className="mb-3", style={"color": TEXT_MAIN, "fontSize": "1.0rem", "fontWeight": "600", "letterSpacing": "1px", "fontFamily": FONT_DISPLAY}), width=8),
        ...
        return html.Div([
            dbc.Row([
                dbc.Col(dbc.Button("BORRAR SELECCION", id="btn-del-sel-hist", color="dark", className="me-2 fw-bold", style={"border": f"1px solid {BORDER_COLOR}", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}), width="auto"), 
                ...
                dbc.Col(
                    dbc.Accordion([
                        dbc.AccordionItem([
                            dcc.ConfirmDialogProvider(
                                children=dbc.Button("BORRAR BASE DE DATOS", id="btn-del-all-hist", color="danger", className="fw-bold w-100", style={"backgroundColor": "red", "color": "white", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}),
        ...
        return html.Div([
            ...
            dcc.Loading(id="loading-analytics", type="default", color=COLOR_NEUTRAL, children=html.Div([
                ...
                dbc.Row([dbc.Col([html.Label("ATRIBUCION POR PARAMETRO:", className="fw-bold mb-2", style={"color": COLOR_NEUTRAL, "letterSpacing": "1px", "fontSize": "0.8rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}), dcc.Dropdown(id='strategy-selector', options=strategy_options, value=default_val, clearable=False, style=DROPDOWN_STYLE)], width=4, className="mb-4")]),
        ...
        return html.Div([
            dbc.Row([
                dbc.Col([html.H4("SIMULADOR DE MONTECARLO", style={"color": TEXT_MAIN, "marginTop": "20px", "fontSize": "1.2rem", "fontWeight": "600", "letterSpacing": "1px", "fontFamily": FONT_DISPLAY}), html.P("Generador de escenarios  basado en distr. de R.", className="text-muted"), html.P("Se recomienda un mínimo de 50/100 operaciones para realizar la simulación.", style={"color": COLOR_NEUTRAL, "fontSize": "0.78rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "marginTop": "-8px"})], width=6),
            ...
            dbc.Button("INICIAR SIMULACION", id="btn-run-mc", color="light", className="w-100 mb-5 fw-bold p-3 text-dark", style={"border": "none", "borderRadius": "4px", "fontSize": "1.1rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}),
        ...
        return dbc.Container([
            ...
            dbc.Row([
                dbc.Col(html.H4("RETORNO ACUMULADO DEL PORTFOLIO",
                               style={"color": TEXT_MAIN, "marginTop": "20px", "fontSize": "1.2rem", "fontWeight": "600", "letterSpacing": "1px", "fontFamily": FONT_DISPLAY}))
            ]),
            
            # Controles: botones de periodo + benchmark + indicador de capital
            dbc.Row([
                dbc.Col([
                    html.Label("Período:", style={"color": COLOR_NEUTRAL, "fontSize": "0.85rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "marginBottom": "5px", "display": "block"}),
                    dbc.ButtonGroup([
                ...
                dbc.Col([
                    html.Label("Benchmark:", style={"color": COLOR_NEUTRAL, "fontSize": "0.85rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "marginBottom": "5px", "display": "block"}),
                    dbc.Select(
                ...
                dbc.Col([
                    html.Div([
                        html.Span("Capital Inicial: ", style={"color": COLOR_NEUTRAL, "fontSize": "0.85rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}),
                        html.Span(f"{cur_sym(conf)}{saved_bal:,.0f}", style={"color": TEXT_MAIN, "fontSize": "0.85rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "fontWeight": "bold"}),
                        html.Span(" (config Analytics)", style={"color": COLOR_NEUTRAL, "fontSize": "0.7rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "marginLeft": "5px"}),
                    ], style={"paddingTop": "25px"})
                ], width=2),

                dbc.Col([
                    html.Div(id="perf-status", style={"color": COLOR_NEUTRAL, "fontSize": "0.85rem", "paddingTop": "25px", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"})
                ], width=3),
                ...
                dbc.Col([
                    ...
                    dbc.Button(
                        ...
                        style={"fontFamily": "'Chakra Petch', 'Inter', sans-serif", "fontWeight": "bold", "borderColor": BORDER_COLOR, "color": COLOR_NEUTRAL}
                    ),
                    dcc.Download(id="download-daily"),
                    dcc.Store(id="perf-daily-store"),
                    dcc.Store(id="perf-period-store"),
            ...
            dbc.Card([
                dbc.CardHeader(
                    dbc.Button(
                        ...
                        style={"color": TEXT_MAIN, "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "letterSpacing": "0.5px"}
                    ),
                ...
                dbc.Collapse(
                    dbc.CardBody([
                        ...
                        dbc.Row([
                            dbc.Col(dbc.Button("ELIMINAR SELECCION", id="btn-del-cm", color="dark", size="sm", style={"fontFamily": "'Chakra Petch', 'Inter', sans-serif", "border": f"1px solid {BORDER_COLOR}"}), width="auto"),
                            dbc.Col(html.Div(id="cm-msg", style={"color": COLOR_NEUTRAL, "fontSize": "0.85rem", "paddingTop": "5px", "fontFamily": "'Chakra Petch', 'Inter', sans-serif"}), width="auto")
                        ], className="mt-2")
                    ], style={"backgroundColor": CARD_BG}),
                    id="collapse-cm",
                    is_open=False
        ...
        return html.Div([
            dbc.Row([
                dbc.Col([
                    html.H3("MANUAL DE USUARIO", className="mb-4", style={"color": TEXT_MAIN, "letterSpacing": "1px", "fontSize": "1.2rem", "fontFamily": FONT_DISPLAY}),
            ...
            dbc.Row([
                dbc.Col(html.Div([
                    ...
                    html.P("Desarrollado por Mirko Gulin", style={"color": COLOR_NEUTRAL, "fontSize": "0.9rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "textAlign": "center", "marginBottom": "5px"}),
                    html.P(html.A("mirkogulin2001@gmail.com", href="mailto:mirkogulin2001@gmail.com", style={"color": COLOR_POS, "textDecoration": "none"}), style={"textAlign": "center", "fontSize": "0.85rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "marginBottom": "15px"}),
                    html.P([
                        "Visitá ",
                    ...
                    ], style={"textAlign": "center", "fontSize": "0.85rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "color": TEXT_MAIN, "marginBottom": "10px"}),
                    html.P("© 2026 Edge Journal", style={"color": BORDER_COLOR, "fontSize": "0.75rem", "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "textAlign": "center", "marginBottom": "0"})
...
def toggle_sl_warning(n, nsl):
    if n and not nsl:
        return {"display": "block", "color": COLOR_NEG, "fontFamily": "'Chakra Petch', 'Inter', sans-serif", "fontSize": "0.78rem",
...
def update_performance(n_ytd, n_yoy, n_all, n_2025, benchmark, n_auto, active_tab, stored_period, session):
    ...
    empty_fig.update_layout(
        ...
        font_color=COLOR_NEUTRAL, font_family="'Chakra Petch', 'Inter', sans-serif",
        xaxis=dict(visible=False), yaxis=dict(visible=False)
    )
    empty_fig.add_annotation(
        text="Sin datos para este período — registrá tus trades en OPERATIVA",
        showarrow=False, xref="paper", yref="paper", x=0.5, y=0.5,
        font=dict(color=COLOR_NEUTRAL, family="Chakra Petch, Inter, sans-serif", size=13)
    )
    ...
    fig_cumulative.update_layout(title={'text': f'RETORNO ACUMULADO (%) — {period_label}', 'font': {'size': 14, 'color': TEXT_MAIN, 'family': 'Chakra Petch, Inter, sans-serif'}, 'x': 0.5, 'xanchor': 'center'}, paper_bgcolor=CARD_BG, plot_bgcolor=CARD_BG, font_color=TEXT_MAIN, font_family="'Chakra Petch', 'Inter', sans-serif", hovermode='x unified', margin=dict(l=60, r=30, t=50, b=10), yaxis=dict(title="Retorno (%)", showgrid=True, gridcolor=BORDER_COLOR, zerolinecolor=BORDER_COLOR, ticksuffix='%'), xaxis=dict(showgrid=False), showlegend=False)
    ...
    fig_drawdown.update_layout(title={'text': 'DRAWDOWN (%)', 'font': {'size': 14, 'color': TEXT_MAIN, 'family': 'Chakra Petch, Inter, sans-serif'}, 'x': 0.5, 'xanchor': 'center'}, paper_bgcolor=CARD_BG, plot_bgcolor=CARD_BG, font_color=TEXT_MAIN, font_family="'Chakra Petch', 'Inter', sans-serif", hovermode='x unified', margin=dict(l=60, r=30, t=40, b=30), yaxis=dict(title="DD (%)", showgrid=True, gridcolor=BORDER_COLOR, zerolinecolor=BORDER_COLOR, ticksuffix='%'), xaxis=dict(title="Fecha", showgrid=False), showlegend=False)
