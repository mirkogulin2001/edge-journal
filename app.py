...
@app.callback(
    ...
    [Input("btn-calc-performance", "n_clicks"), Input("tabs", "value")],
    [State("perf-initial-balance", "value"), State("session-store", "data")]
)
def update_performance(n_clicks, active_tab, initial_balance, session):
    ...
    colors = [COLOR_POS if r >= 0 else COLOR_NEG for r in ret_pct]
